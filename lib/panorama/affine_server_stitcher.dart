...
import 'pano_stitcher.dart' show StitchResult;
import 'pano_tile_image.dart';

/// Phase 1-4 — the affine StitchMode. Mirrors [PanoStitcher.stitch]'s shape
/// (same args, same [StitchResult]) but instead of the OpenPano JNI call it
/// drives the in-process panostitch-renderer over its localhost HTTP/GraphQL
/// surface: tiles go up via `POST /upload`, a `stitch` mutation kicks off the
/// Chaquopy/Python `AffineStitcher`, the `stitchEvents` WebSocket subscription
/// streams live progress, and the panorama PNG comes back from
/// `GET /result/{stitchId}`.
///
/// The backend runs in-process (started in MainActivity); its port is obtained
/// once via the `…/backend` MethodChannel.
class AffineServerStitcher {
  AffineServerStitcher({MethodChannel? channel})
      : _channel =
  ...
  /// Stitch [tilesRowMajor] via the server. [onStage] reports a coarse stage
  /// label + [0,1] progress for the overlay, same contract as [PanoStitcher].
  Future<StitchResult> stitch(
    ...
    try {
      ...
      final stitchId = await _mutateStitch(base, uploadIds, nCols);
  ...
  Future<String> _mutateStitch(Uri base, List<String> uploadIds, int nCols) async {
    const query =
        r'mutation($ids:[ID!]!,$n:Int){ stitch(uploadIds:$ids, nCols:$n) }';
    final res = await http.post(
      base.resolve('/graphql'),
      headers: const {'Content-Type': 'application/json'},
      body: jsonEncode({
        'query': query,
        'variables': {'ids': uploadIds, 'n': nCols},
      }),
    );
  ...
  /// Same tile→PNG rendering as [PanoStitcher]: draw the tile's `src`
